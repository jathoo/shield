<?php foreach ( $posts as $post ) {
	$quick = (isset($post->tags['quick'])) ? true: false;
	?>
	<div id="entry-<?php echo $post->id; ?>" class="multi post entry-multi-list <?php echo $post->statusname; ?><?php if($quick) { ?> quick<?php } ?><?php if($post->content_type == Post::type('link')): ?> link<?php endif; ?>">
		<div class="head">
			<span class="date">
			...
			<?php if($post->content_type == Post::type('link')): ?><h3 class="title"><a href="<?php echo $post->link; ?>" title="Visit link"><?php echo $post->title_out; ?></a></h3>
			<?php else: ?><h3 class="title"><a href="<?php echo $post->permalink; ?>" title="Visit <?php echo $post->title; ?>"><?php echo $post->title_out; ?></a></h3><?php endif; ?>
			<?php if(isset($post->tags['quick'])) { ?><a class="quick" href="<?php echo URL::get('display_entries_by_tag', array('tag' => 'quick')); ?>" title="Read more quick entries">quick</a><?php } ?>
			<?php if($post->content_type == Post::type('link')): ?><a class="link" href="<?php echo $post->permalink; ?>" title="Get the post permalink">permalink</a><?php endif; ?>
		</div>
		<div class="meta">
			<?php if(!isset($post->tags['quick'])) { ?>
			<?php if($post->content_type != Post::type('link')): ?><abbr class="time section" title="<?php echo $post->pubdate->get( 'g:i a \o\n F jS, Y' ); ?>"><?php echo $post->pubdate->get( 'h:i A' ); ?></abbr>
